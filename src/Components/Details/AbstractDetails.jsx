...
function AbstractDetails() {
  ...
  return (
    ...
    <Link to={'/abstract'}><HiXMark className='absolute right-20 top-10 text-2xl z-10'/></Link>
    ...
    {abstracts.map((value,index)=>{
      if(value.id==image){
            ...
            <div className='px-32 flex flex-wrap w-full   h-screen '>
                  ...
                  <GrFormNext className={`absolute top-1/2 lg:right-20 right-10 text-4xl ${image == abstracts.length-1 ? "lg:text-slate-200 text-slate-600":"text-black-300"}`} onClick={()=>handleNext(index+1)}/>
